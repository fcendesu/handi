...
<head>
    ...
    <title>Yeni Ödeme Yöntemi - {{ config('app.name', 'Handi') }}</title>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <div class="flex items-center mb-6">
                        ...
                        <h1 class="text-3xl font-bold text-gray-900">Yeni Ödeme Yöntemi Ekle</h1>
                    </div>
                    ...
                    <form action="{{ route('payment-methods.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                ...
                                placeholder="Bu ödeme yöntemi hakkında ek bilgiler...">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
